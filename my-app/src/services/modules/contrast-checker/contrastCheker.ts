...
export const checkContrast = async (url: string): Promise<ModuleCheckResult[]> => {
  ...
  // Собираем только цвета и размеры шрифта в браузере
  ...
  // Вычисляем контраст в Node.js
  const results: ModuleCheckResult[] = elementsData.map(el => {
    const ratio = getContrastRatio(rgbToHex(el.textColor), rgbToHex(el.bgColor));
    const fontSize = parseFloat(el.fontSize);
    const isLargeText = fontSize >= 18 || (fontSize >= 14 && el.fontWeight === 'bold');
    ...
    return {
      ...
      item: el.tag.toLowerCase() + (el.id ? `#${el.id}` : '') + (el.className ? `.${el.className}` : ''),
