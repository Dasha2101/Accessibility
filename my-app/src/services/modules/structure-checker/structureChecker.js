export const checkStructure = async (page, options) => {
    try {
        const totalElements = await page.evaluate(() => document.body.querySelectorAll('*').length);
        const results = await page.evaluate(() => {
            ...
            headers.forEach((el) => {
                ...
                if (lastLevel && level - lastLevel > 1) {
                    addResult(el.tagName.toLowerCase(), `Пропущен уровень заголовка (предыдущий h${lastLevel})`, 'warning');
                }
                lastLevel = level;
            });
            ...
            lists.forEach((list) => {
                Array.from(list.children).forEach((li) => {
                    if (li.tagName.toLowerCase() !== 'li') {
                        addResult(list.tagName.toLowerCase(), 'Список содержит элемент, не являющийся <li>', 'error');
                    }
                });
            });
            ...
            tables.forEach((table) => {
                if (!table.querySelector('thead') || !table.querySelector('tbody')) {
                    addResult('table', 'Таблица должна содержать <thead> и <tbody>', 'warning');
                }
                if (table.querySelectorAll('th').length === 0) {
                    addResult('table', 'Таблица должна содержать хотя бы один <th>', 'warning');
                }
            });
            ...
            inputs.forEach((input) => {
                ...
                const hasLabel = (id && document.querySelector(`label[for="${id}"]`)) ||
                    input.closest('label');
                if (!hasLabel) {
                    addResult(input.tagName.toLowerCase(), 'Поле формы без связанного <label>', 'error');
                }
            });
            const ariaElements = elements.filter((el) => el.hasAttribute('role') ||
            ...
            ariaElements.forEach((el) => {
                ...
                if (role === 'presentation' && el.children.length > 0) {
                    addResult(el.tagName.toLowerCase(), 'role="presentation" с вложенными элементами может скрывать контент', 'warning');
                }
            });
            if (results.length === 0) {
                results.push({
